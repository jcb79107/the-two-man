...
function resolveSelectionHoles(
  tee: {
    id: string;
    name: string;
    gender: string;
  ...
  return hydrateTeeHolesFromSiblings(tee, courseTees).map((hole) => ({
...
export async function POST(
  ...
  await assertRequestAllowed({
    ...
    limit: 60,
    windowSeconds: 60 * 10,
    message: "Too many scorecard updates. Please pause for a minute and try again."
  });
  ...
  try {
    ...
    if (action === "setup") {
      ...
      const setupConfirmed = body?.setupConfirmed === true;
      ...
      if (!setupConfirmed) {
        return badRequest(
          "Confirm today’s Handicap Indexes and tee selections for all four players before opening the scorecard."
        );
      }
      ...
      await db.$transaction(async (tx) => {
        const selectedHoleTemplateByTeeId = new Map(
          inputPlayers.map((player) => [player.teeId, player.holes])
        );

        for (const [teeId, holes] of selectedHoleTemplateByTeeId.entries()) {
          const tee = teeById.get(teeId);

          if (!tee || tee.holes.length === 18 || holes.length !== 18) {
            continue;
          }
      ...
      const holeTemplate =
        match.playerSelections[0] == null
          ? []
          : resolveSelectionHoles(
              match.playerSelections[0].tee,
              match.course?.tees ?? match.playerSelections.map((selection) => selection.tee)
            );
      ...
      await db.$transaction(async (tx) => {
        ...
        if (action === "publish") {
          const inputs: MatchPlayerInput[] = match.playerSelections.map((selection) => ({
            ...
            holes: resolveSelectionHoles(
              selection.tee,
              match.course?.tees ?? match.playerSelections.map((candidate) => candidate.tee)
            )
          }));
