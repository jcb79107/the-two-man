...
const RESPONSES_CSV =
  process.argv[2] ??
  "/Users/jason/Downloads/2026 Individual or Two Man Match Play Bracket Opt-In (Responses) - Form Responses 1.csv";
const PODS_CSV =
  process.argv[3] ??
...
function cleanEmail(value: string) {
  const normalized = normalizeWhitespace(value).replace(/,+$/, "").trim();
...
function parseResponses() {
  ...
  return new Map(
    rows.map((row) => {
      ...
      const playerOneEmail = cleanEmail(row[2] ?? "");
      const playerTwoName = normalizeName(row[3] ?? "");
      const playerTwoEmail = cleanEmail(row[4] ?? "");
      const teamName = buildTeamName(playerOneName, playerTwoName);

      return [
        canonicalizeTeamName(teamName),
        {
          teamName: TEAM_NAME_OVERRIDES.get(canonicalizeTeamName(teamName)) ?? teamName,
          players: [
            {
