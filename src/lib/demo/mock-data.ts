...
const TEAM_PLAYERS = [
  ["Avery Anderson", "Blake Brooks"],
  ["Casey Campbell", "Drew Diaz"],
  ["Emerson Evans", "Finley Foster"],
  ["Gray Garcia", "Harper Hayes"],
  ["Indy Iverson", "Jules Jenkins"],
  ["Kai King", "Logan Lewis"],
  ["Morgan Mitchell", "Nico Nguyen"],
  ["Oakley Owens", "Parker Parker"],
  ["Quincy Quinn", "Reese Rivera"],
  ["Sawyer Simmons", "Taylor Turner"],
  ["Uma Underwood", "Vale Valdez"],
  ["Wren Walker", "Yael Young"],
  ["Bailey Bennett", "Corey Carter"],
  ["Devon Dawson", "Emery Ellis"],
  ["Flynn Franklin", "Greer Griffin"],
  ["Hayden Hudson", "Indigo Ingram"],
  ["Lane Lawson", "Micah Monroe"],
  ["Nova Norris", "Peyton Preston"]
] as const;
const POD_TEAM_NUMBERS = [
  [1, 13, 15],
  [2, 4, 17],
  [8, 5, 7],
  [3, 16, 6],
...
export const demoPods: PodProfile[] = POD_LETTERS.map((letter, index) => ({
  ...
  name: `Pod ${letter}`,
  order: index + 1,
  teamIds: POD_TEAM_NUMBERS[index].map((teamNumber) => `team-${String(teamNumber).padStart(2, "0")}`)
}));
...
function splitName(fullName: string) {
  ...
  return {
    ...
    displayName: parts.join(" ") || fullName
  };
}
...
export const demoTeams: TeamProfile[] = TEAM_NAMES.map((teamName, index) => {
  ...
  const podIndex = POD_TEAM_NUMBERS.findIndex((podTeams) =>
    (podTeams as readonly number[]).includes(teamNumber)
  );
  const podLetter = POD_LETTERS[podIndex >= 0 ? podIndex : Math.floor(index / 3)];
  const [playerOneName, playerTwoName] = TEAM_PLAYERS[index];
  const playerOne = splitName(playerOneName);
  const playerTwo = splitName(playerTwoName);
