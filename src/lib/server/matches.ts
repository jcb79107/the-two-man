...
import { getStoredCourseCatalog, hydrateTeeHolesFromSiblings } from "@/lib/server/course-catalog";
    ...
    const selectionByPlayerId = new Map(
      match.playerSelections.map((selection) => [
        selection.playerId,
        {
          ...
          holes: resolveSelectionHoles(selection.tee, match.course?.tees ?? [selection.tee])
        }
      ])
    );
