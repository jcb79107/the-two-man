...
const DEFAULT_USGA_LOOKUP_SCRIPT =
  process.env.USGA_LOOKUP_SCRIPT ??
  "/Users/jason/Developer/archive/index-web/scripts/usga_lookup_api.py";
const SCRAPE_ENV = {
  ...process.env,
...
export class UsgaScrapeCourseDirectoryProvider implements CourseDirectoryProvider {
  async searchCourses(query: CourseLookupQuery): Promise<CourseLookupResult[]> {
    ...
    const { stdout } = await execFileAsync("python3", [DEFAULT_USGA_LOOKUP_SCRIPT, ...args], {
      cwd: dirname(DEFAULT_USGA_LOOKUP_SCRIPT),
      env: SCRAPE_ENV,
