...
export function PrivateMatchWorkspace({
  ...
  const setupBlockers = [
    ...
    !setupConfirmed
      ? "Confirm that these are today’s Handicap Indexes and tee selections before opening the scorecard."
      : null
  ...
  const scorecardGridClass =
    scoreSegment === "front"
      ? isCompactScorecard
        ? "grid grid-cols-[90px_repeat(9,minmax(28px,1fr))_44px] sm:grid-cols-[160px_repeat(9,minmax(78px,1fr))_96px]"
        : "grid grid-cols-[160px_repeat(9,minmax(78px,1fr))_96px]"
      : isCompactScorecard
        ? "grid grid-cols-[90px_repeat(9,minmax(28px,1fr))_44px_44px] sm:grid-cols-[160px_repeat(9,minmax(78px,1fr))_96px_96px]"
  ...
  const tableLabelCellClass = isCompactScorecard
    ? "px-1.5 py-1.5 text-[9px] leading-3.5 sm:px-4 sm:py-4 sm:text-lg"
    : "px-4 py-4 text-lg";
  const tableBodyCellClass = isCompactScorecard
    ? "px-0.5 py-1 text-center sm:px-3 sm:py-3"
    : "px-3 py-3 text-center";
  const scoreCircleSizeClass = isCompactScorecard
    ? "h-5 w-5 text-[11px] leading-none tracking-[-0.02em] sm:h-12 sm:w-12 sm:text-xl"
    : "h-12 w-12 text-xl";
  const baseHoleMetaByNumber = new Map(
    (holesByPlayerId.get(data.players[0]?.playerId ?? "") ?? []).map((hole) => [hole.holeNumber, hole])
  );
  const selectedHoleMeta = baseHoleMetaByNumber.get(selectedHoleNumber);
  const selectedHoleCompletedCount = teamGroups.reduce((total, team) => {
    return total + team.players.filter((player) => {
      const value = scoreRows.find((hole) => hole.holeNumber === selectedHoleNumber)?.scores[player.playerId] ?? "";
      return value !== "";
    }).length;
  }, 0);
  const selectedHoleStrokeSummary = data.setupPreview?.players
    .filter((player) => (player.strokesByHole[selectedHoleNumber] ?? 0) > 0)
    .map((player) => `${player.playerName} (${player.strokesByHole[selectedHoleNumber]})`)
    ?? [];
    ...
    <div className="space-y-4">
      ...
      {pageMode === "setup" ? (
        <section className="rounded-[28px] border border-mist bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            ...
            <p className="mt-2 text-sm leading-6 text-ink/72">Enter all four current indexes, then pick the course and tees. Before opening the card, confirm these are the day-of values everyone agreed to use.</p>
            </div>
          {data.setupComplete ? (
            <button
              type="button"
              ...
              className="rounded-full border border-fairway/15 bg-white px-4 py-2 text-sm font-medium text-ink disabled:cursor-not-allowed disabled:opacity-60"
            >
              Open scorecard
            </button>
          ) : null}
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <div className="rounded-[24px] border border-dashed border-fairway/16 bg-sand p-4 sm:col-span-2">
            ...
            <p className="mt-3 text-sm leading-6 text-ink/68">
              Search pulls course, tee, rating, slope, and scorecard-row data into The Two Man.
        ...
        {!courseLoaded ? (
          ...
          <>
            ...
            {!data.isPublished ? (
              <div className="mt-4 rounded-[24px] border border-[#d8c27a] bg-[#fff7dd] p-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#8a6a09]">
                  Setup confirmation
                </p>
                <label className="mt-3 flex items-start gap-3 rounded-2xl border border-[#ead59a] bg-white px-4 py-4 text-sm leading-6 text-ink">
                  <input
                    type="checkbox"
                    checked={setupConfirmed}
                    onChange={(event) => setSetupConfirmed(event.target.checked)}
                    className="mt-1 h-4 w-4 rounded border-[#c7b37f] text-pine focus:ring-pine"
                  />
                  <span>
                    We confirmed these are today’s Handicap Indexes and tee selections for all four players before opening the scorecard.
                  </span>
                </label>
                <p className="mt-3 text-xs leading-5 text-ink/68">
                  If you change the course, any tee, any Handicap Index, or any manual hole row, you will need to confirm again.
                </p>
              </div>
            ) : null}
      ...
      {data.setupComplete && pageMode === "scorecard" ? (
        <section className="overflow-hidden rounded-[28px] border border-[#d7c28d] bg-[#f7efd8] shadow-[0_18px_40px_rgba(76,58,26,0.14)]">
          ...
          <div className="mx-4 mt-4 rounded-[24px] border border-[#d8c27a] bg-[#fff7dd] p-4 text-sm leading-6 text-ink/78">
            This scorecard is based on the saved setup values for course, tees, Handicap Indexes, and any manual hole rows. If anything looks wrong, use <span className="font-semibold text-ink">Edit setup</span> before entering or submitting scores.
          </div>
          ...
          <div className="space-y-4 px-4 py-4">
            ...
            <div className="rounded-[28px] border border-[#d7c28d] bg-white shadow-[0_12px_28px_rgba(76,58,26,0.08)]">
              <div className="border-b border-[#e9d8ac] bg-[#fbf5e6] px-4 py-3">
                ...
                <div className="mt-3 rounded-[20px] border border-[#e4d6b5] bg-white px-3 py-3 text-sm text-ink/74">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="font-semibold text-ink">
                      Hole {selectedHoleNumber}
                      {selectedHoleMeta ? ` • Par ${selectedHoleMeta.par} • Hdcp ${selectedHoleMeta.strokeIndex}` : ""}
                    </p>
                    <p className="text-xs uppercase tracking-[0.16em] text-fairway/62">
                      {selectedHoleCompletedCount}/{data.players.length} scores entered
                    </p>
                  </div>
                  <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
                    {visibleHoleNumbers.map((holeNumber) => {
                      const completedForHole = data.players.filter((player) => {
                        const value = scoreRows.find((hole) => hole.holeNumber === holeNumber)?.scores[player.playerId] ?? "";
                        return value !== "";
                      }).length;
                      const isActive = holeNumber === selectedHoleNumber;

                      return (
                        <button
                          key={`hole-jump-${holeNumber}`}
                          type="button"
                          onClick={() => setSelectedHoleNumber(holeNumber)}
                          className={`rounded-full px-3 py-2 text-xs font-semibold transition ${
                            isActive
                              ? "bg-pine text-white"
                              : completedForHole === data.players.length
                                ? "border border-pine/20 bg-[#eef8f1] text-pine"
                                : completedForHole > 0
                                  ? "border border-[#d7c28d] bg-[#fff7dd] text-[#8a6b08]"
                                  : "border border-[#d7c28d] bg-white text-ink/74"
                          }`}
                        >
                          {holeNumber}
                        </button>
                  ...
                  {selectedHoleStrokeSummary.length > 0 ? (
                    <p className="mt-3 text-xs leading-5 text-ink/62">
                      Strokes on this hole: {selectedHoleStrokeSummary.join(", ")}
                    </p>
                  ) : null}
                </div>
              </div>
                      ...
                      <Fragment key={`team-block-${teamCard.team.teamId}`}>
                        {teamCard.players.map((playerCard) => (
                          <Fragment key={`${teamCard.team.teamId}-${playerCard.player.playerId}`}>
                            <div className={`border-b border-r border-[#d7c28d] bg-white ${isCompactScorecard ? "px-2 py-1.5 sm:px-4 sm:py-4" : "px-4 py-4"}`}>
                              <p className={`${isCompactScorecard ? "text-[10px] leading-4 sm:text-sm" : "text-sm"} font-semibold text-ink`}>
                                {isCompactScorecard ? playerCard.player.playerName.split(" ")[0] : playerCard.player.playerName}
                              </p>
                              {isCompactScorecard ? null : (
                                <div className="mt-1.5 flex items-center gap-2 text-xs text-ink/56">
                              ...
                              return (
                                ...
                                >
                                  <div className="relative mx-auto w-fit">
                                    {strokeCount > 0 && !isCompactScorecard ? (
                                      <span
                                    ...
                                    {isScorecardReadOnly ? (
                                      <div
                                        className={`mx-auto flex items-center justify-center font-semibold ${scoreCircleSizeClass} ${scoreStyleForValue(
                                          String(score),
                                          holeMeta?.par,
                                          isCompactScorecard
                                        )}`}
                                        aria-label={`Hole ${holeNumber} score ${score || "not entered"}`}
                                      >
                                        {score || "-"}
                                      </div>
                                    ) : (
                ...
                {canAdminOverridePostedCard ? (
                  <div className="space-y-2">
                    ...
                    <div className="space-y-3">
                      ...
                      <p className="text-sm text-ink/72">
                        Read the full card together, then tick each player once everyone agrees the posted scores are final.
                      </p>
