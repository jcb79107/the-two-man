...
export function AdminMatchOpsList({ rows }: AdminMatchOpsListProps) {
  ...
  const canArchiveRow = (row: AdminMatchOpsRow) => row.statusCode === "FINAL" || row.statusCode === "FORFEIT";
  const allActiveRows = rows.filter((row) => !archivedSet.has(row.id) || !canArchiveRow(row));
  const archivedRows = rows.filter((row) => archivedSet.has(row.id) && canArchiveRow(row));
    ...
    return (
      ...
      >
        ...
        <div className="mt-3">
          ...
          {row.overrideNote ? (
            <p className="mt-1 text-xs text-[#5b4696]">Admin note: {row.overrideNote}</p>
          ) : null}
          {sentAt ? (
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          ...
          >
            {isPublishedResult ? "View admin card" : "Open"}
          </Link>
          {row.publicUrl ? (
            <Link
              href={row.publicUrl}
          ...
          <CopyButton value={row.privateUrl} label="Copy link" />
          <CopyButton value={row.inviteMessage} label="Copy invite" />
          {row.emailInviteHref ? (
            <EmailInviteButton
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="rounded-[20px] border border-mist bg-white px-4 py-3 text-xs text-ink/62">
        Sorted by urgency: waiting field, missing email, live edits, unsent ready matches, then completed cards. Only completed cards can be archived, so active work cannot disappear by accident.
      </div>

      <div className="grid gap-3 md:grid-cols-3">
        <div className="rounded-[20px] border border-[#d7c28d] bg-[#fff7dd] px-4 py-3 text-sm text-ink/72">
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8a6b08]">Operator order</p>
          <p className="mt-2">Work left to right: send unsent ready links, monitor live cards, then archive completed cards.</p>
        </div>
        <div className="rounded-[20px] border border-[#cabaf3] bg-[#f6f1ff] px-4 py-3 text-sm text-ink/72">
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#5f47a6]">Email coverage</p>
          <p className="mt-2">Missing email means someone in the group still cannot be reached from the quick-send tools.</p>
        </div>
        <div className="rounded-[20px] border border-pine/15 bg-[#eef8f1] px-4 py-3 text-sm text-ink/72">
          <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-pine">Invite tools</p>
          <p className="mt-2">Copy the private link or the full invite text before you jump into Gmail, text, or a group chat.</p>
        </div>
      </div>
