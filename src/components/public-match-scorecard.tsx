...
function scoreShape(score: number | null | undefined, par: number) {
  ...
  if (delta <= -2) {
    return "rounded-full border-[4px] border-ink text-ink shadow-[inset_0_0_0_2px_#fffaf0] [border-style:double]";
  }

  if (delta === -1) {
    return "rounded-full border-[3px] border-ink text-ink";
  }

  if (delta === 1) {
    return "rounded-[4px] border-[3px] border-ink text-ink";
  }

  if (delta >= 2) {
    return "rounded-[4px] border-[4px] border-ink text-ink shadow-[inset_0_0_0_2px_#fffaf0] [border-style:double]";
  }
  ...
  return (
    <div className="mx-auto w-full max-w-[430px] space-y-4 md:max-w-[620px]">
      ...
      <section className="overflow-hidden rounded-[28px] border border-white/70 bg-[linear-gradient(145deg,rgba(255,252,247,0.98),rgba(247,241,227,0.94))] p-4 shadow-[0_14px_34px_rgba(17,32,23,0.09)]">
        ...
        <p className="mt-4 text-base font-medium leading-6 text-ink/64">
          {courseName} • {playedOnLabel}
      ...
      <section className="rounded-[26px] border border-mist bg-white/92 p-4 shadow-[0_14px_34px_rgba(17,32,23,0.08)]">
        <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-[#c5a250]">
          {courseName}
        </p>
          ...
          {(["front", "back"] as Segment[]).map((item) => (
            <button
              ...
              className={`min-h-12 rounded-full border px-4 text-base font-semibold transition ${
                segment === item ? "border-pine bg-pine text-white" : "border-mist bg-white text-ink/58"
              }`}
            >
        ...
        <div className="-mx-4 mt-4 overflow-x-auto overscroll-x-contain px-4 pb-2">
          <div
            className="overflow-hidden rounded-[24px] border border-mist bg-white"
            style={{
          ...
          >
            ...
            >
              <div className="sticky left-0 z-10 border-b border-r border-[#d7c28d] bg-[#f2ead9] px-3 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-ink/58">
                Hole
              </div>
              {activeHoles.map((hole) => (
                <div key={hole.holeNumber} className="border-b border-r border-mist px-1 py-2 text-center">
                  <span className="mx-auto flex h-8 w-8 items-center justify-center rounded-full bg-pine/10 text-sm font-semibold text-ink">
                    {hole.holeNumber}
                  </span>
                </div>
              ))}
              <div className="border-b border-r border-mist px-2 py-3 text-center text-sm font-semibold text-ink">
                ...
                <div key={label as string} className="contents">
                  ...
                  {(values as Array<number | string>).map((value, index) => (
                    <div
                      ...
                      className="border-b border-r border-mist px-1 py-3 text-center text-sm font-semibold text-ink/64"
                    >
                      {value}
                    </div>
                  ))}
                  ...
                  <div key={team.id} className="contents">
                    {team.players.map((player) => (
                      <div key={player.playerId} className="contents">
                        <div
                          className={`sticky left-0 z-10 border-b border-r border-[#d7c28d] bg-[#fffaf0] px-3 py-3 ${colors.text}`}
                        >
                        ...
                        {activeHoles.map((hole) => {
                          ...
                          return (
                            <div
                              ...
                              className={`border-b border-r border-mist px-1 py-2 text-center ${
                        ...
                        <div className="border-b border-r border-mist bg-[#f7f1e3] px-2 py-3 text-center text-base font-semibold text-ink">
                          {sumPlayerGross(player, activeHoles)}
                        </div>
                        {showRoundTotal ? (
                          <div className="border-b border-r border-mist bg-[#efe7d6] px-2 py-3 text-center text-base font-semibold text-ink">
                    ...
                    <div className={`sticky left-0 z-10 border-b border-r border-[#d7c28d] px-3 py-3 ${colors.soft}`}>
                    ...
                    {activeHoles.map((hole) => {
                      ...
                      return (
                        <div
                          ...
                          className="border-b border-r border-mist bg-white px-1 py-2 text-center"
                        >
                          <span
                    ...
                    <div className={`border-b border-r border-mist px-2 py-3 text-center text-base font-semibold ${colors.soft} text-ink`}>
                      {teamTotal}
                    </div>
                    {showRoundTotal ? (
                      <div className={`border-b border-r border-mist px-2 py-3 text-center text-base font-semibold ${colors.soft} text-ink`}>
